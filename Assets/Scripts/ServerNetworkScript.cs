...
class Messages{
	...
	public static byte[] CreateSetAvatarPositionRotationMessage(
		...
		byte[] xByte = BitConverter.GetBytes((short)(x * 100f));
		m[3] = xByte[0];
		m[4] = xByte[1];

		byte[] zByte = BitConverter.GetBytes((short)(z * 100f));
		m[5] = zByte[0];
		m[6] = zByte[1];

		byte[] rByte = BitConverter.GetBytes((short)(r * 100f));
		m[7] = rByte[0];
		m[8] = rByte[1];
		//--------------------------------------------
...
public class ServerNetworkScript : MonoBehaviour {
	...
	void AddNewClient(int recHostId, int connectionId, int channelId) {
		...
		byte[] setPlayerIdMessage = Messages.CreateSetPlayerIdMessage(connectionId);
		byte error;
		NetworkTransport.Send(recHostId, connectionId, channelId, setPlayerIdMessage, setPlayerIdMessage.Length, out error);
		Messages.LogNetworkError(error);

		byte[] useAvatarMessage = Messages.CreateUseAvatarMessage(avatarForThisConnection);
		NetworkTransport.Send(recHostId, connectionId, channelId, useAvatarMessage, useAvatarMessage.Length, out error);
		Messages.LogNetworkError(error);
		//--------------------------------
	}
